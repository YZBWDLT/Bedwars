...
export const updateLog = {
    ...
    version1_1: [
        ...
        "§7- 新增以下设置：地图清除速度、地图加载速度、玩家人数上限、玩家自主选队、资源生成间隔（各种资源）、击杀样式设置、无效队伍检测、虚空可扔物品、备份与恢复设置、开发者设置。",
        ...
        "§7- 只允许一个队伍比另一个队伍最多多出 1 名玩家§7（比如不允许3 3 3 1的组队方法）。", // debug 应该显示有哪些玩家选择了该队伍
        "§l新地图",
    ...
    versionTest: [
        ...
        "§l性能优化",
        "§7- §l§b重要更新§r§7 全面重写脚本底层，以应对Alpha 1.1_01版本中糟糕的性能问题表现。",
        "§7- 我们预期，该版本的性能优化理论上应该比 1.0 版本要更好。",
        "§7- 大幅度优化了商店的性能和表现",
        "§l游戏内容",
        "§7- 将包图标改成了床。",
        "§7- 将创造模式的判断准则改为了判断是否为管理员玩家。",
        "§l商店",
        "§7- §l§b重要更新§r§7 现在全新的商店设计，商人 UI 现在采用非共享 UI，不同的玩家将看到不同的内容。",
        "§7- 将以前的“方块与物品”和“武器与盔甲”商人合二为一，成为物品商店，物品商店采用类似于 Hypixel 的标签页设计，包含 9 大分类：快速购买、方块、近战、盔甲、工具、远程、药水、实用道具、轮换物品。",
        "§7- 现在镐子和斧头不再摊开放置了，而是在一个格子之内。",
        "§7- 现在除非和商人交互，否则不会再误锁物品栏，并且只要和商人交易后小角度切换视角即可解锁物品栏。",
        "§7- 修复了梯子定价为 4 个金锭的问题，改为了 4 个铁锭。",
        "§7- 将商店的高等级盔甲改为了靴子（以前是胸甲）。",
        "§7- 将“这是个陷阱！”更名为“失明陷阱”；将“报警陷阱”更名为“显影陷阱”；移除了“末影龙增益”团队升级，改为了“缓冲靴子”。",
        "§7- 现在商店物品的提示框也和 Hypixel 保持了高度一致。",
        "§7- 现在团队升级在已解锁后或陷阱排满后会显示出来，而不是显示为花费。",
        "§l结构加载",
        "§7- §l§b重要更新§r§7 结构加载现在是一个一个岛屿地加载（甚至是一个岛屿的一部分一部分地加载），而不再是所有结构同时加载",
        "§7- 现在每个结构都有自己的加载时间，如果结构大小比较大，加载时间就会比较长，反之则会比较短",
        "§7- 采用这种异步加载方式，对于某些较小的地图来说，这种加载方式可以兼顾加载速度和性能，对于某些较大的地图来说，虽然加载速度会变慢，但是理论上应该可以在结构加载时提供更好的服务端性能",
        "§l设置",
        "§7- 更新了设置页面，现在设置页面有大标题、文本解释和分割线了",
        "§7- 现在生成地图界面显示地图的中文名",
        "§7- 现在在设置清除地图和加载地图速度为较快、快和非常快后，清除和加载速度分别会达到默认状态的 150%%、200%% 和 400%%",
        "§7- 新增了一个新设置：杂项设置 - 备份与恢复设置，用于在应用设置时保存设置内容，并在/reload、重启地图、重启服务器后恢复到以前应用的设置",
        "§7- 在设置页面新增了更新日志界面，可查看该版本和过往版本的更新内容",
        "§7- 更新了开发者设置的内容，现在仅开放一项设置：开启调试模式",
        "§l物品",
        "§7- 搭桥蛋现在不再会将已搭好的桥清空掉某些部分，也不再能在安全区放置方块了",
        "§7- 现在梦境守护者的使用能够适应玩家所交互的方块面，并降低了梦境守护者的伤害",
        "§7- §l§b重要更新§r§7 现在爆炸施加向量采用新式算法：以前，爆炸只会对玩家施加 y 方向上的速度，具体表现为抬高，但不添加水平方向的速度；现在，爆炸根据爆炸位置和玩家位置的连线向量进行计算，这意味着爆炸将带来更加强力而真实的冲击效果",
    ],
}
