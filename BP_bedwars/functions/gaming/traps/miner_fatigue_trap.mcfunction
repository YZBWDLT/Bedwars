...
# --- 为踏入陷阱的玩家施加效果 ---
execute as @a[scores={teamUpgrade=0},r=10] at @s unless score @s team = @e[r=10,family=bed,c=1] team run effect @s mining_fatigue 10 0 true
...
execute as @a if score @s team = @e[r=1,family=bed] team run titleraw @s subtitle {"rawtext":[{"translate":"style.hypixel.trap_triggered.subtitle","with":{"rawtext":[{"translate":"team_upgrade.miner_fatigue_trap"}]}}]}
