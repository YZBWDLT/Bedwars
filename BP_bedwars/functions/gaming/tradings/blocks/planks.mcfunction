...
# 先给予木板，再清除
give @s[hasitem={item=bedwars:gold_ingot,quantity=4..}] planks 16

playsound note.pling @s[hasitem={item=bedwars:gold_ingot,quantity=4..}] ~~~ 0.5 2
tellraw @s[hasitem={item=bedwars:gold_ingot,quantity=4..}] {"rawtext":[{"translate":"system.tradings.succeed","with":{"rawtext":[{"translate":"item.planks"}]}}]}

# 清除
clear @s[hasitem={item=bedwars:gold_ingot,quantity=4..}] bedwars:gold_ingot -1 4

# --- 清除标记物品 ---
clear @s bedwars:shopitem_planks