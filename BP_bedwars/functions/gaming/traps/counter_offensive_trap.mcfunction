...
# --- 提示本队玩家陷阱已触发 ---
execute as @a if score @s team = @e[r=1,family=bed] team run titleraw @s times 10 60 60
execute as @a if score @s team = @e[r=1,family=bed] team run titleraw @s title {"rawtext":[{"translate":"style.hypixel.trap_triggered.title"}]}
execute as @a if score @s team = @e[r=1,family=bed] team run titleraw @s subtitle {"rawtext":[{"translate":"style.hypixel.trap_triggered.subtitle","with":{"rawtext":[{"translate":"team_upgrade.counter_offensive_trap"}]}}]}
execute as @a if score @s team = @e[r=1,family=bed] team at @s run playsound mob.shulker.teleport @s ~~~ 1 0.5
execute as @a if score @s team = @e[r=1,family=bed] team run effect @s speed 15 1 true
execute as @a if score @s team = @e[r=1,family=bed] team run effect @s jump_boost 15 1 true
