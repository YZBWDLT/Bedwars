...
execute if entity @s[scores={temp=..1}] run tellraw @s {"rawtext":[{"translate":"system.tradings.need_equipment","with":{"rawtext":[{"translate":"team_upgrade.forge_tier_2"}]}}]}

# --- 当该队伍无该升级时 ---

## 当该玩家物资不足时
execute if entity @s[scores={temp=2},hasitem={item=bedwars:diamond,quantity=..11}] run playsound mob.shulker.teleport @s ~~~ 1 0.5
execute if entity @s[scores={temp=2},hasitem={item=bedwars:diamond,quantity=..11}] run tellraw @s {"rawtext":[{"translate":"system.tradings.need_more_resources"}]}

## 当该玩家物资充足时
execute if entity @s[scores={temp=2},hasitem={item=bedwars:diamond,quantity=12..}] run function gaming/tradings/team_upgrade/forge_tier_3_2

# --- 清除标记物品 ---
clear @s bedwars:upgrade_forge_tier_3