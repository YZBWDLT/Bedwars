...
/** 获取实体所有物品栏的物品
 * @param {Entity} entity 待获取的实体
 * @returns { { item: ItemStack; slot: number; }[] }
 */
export function getValidItems( entity ) {
    return getAllInventoryItems( entity ).filter( itemInfo => itemInfo.item !== undefined );
}
...
export function eachValidItem( entity, func ) {
    getValidItems( entity ).forEach( itemInfo => { func( itemInfo.item, itemInfo.slot ) } );
};
