...
# --- 为踏入陷阱的玩家施加效果 ---
execute as @a[scores={teamUpgrade=0},r=10] at @s unless score @s team = @e[r=10,family=bed,c=1] team run effect @s invisibility 0 1 true

# --- 提示本队玩家陷阱已触发 ---
execute as @a if score @s team = @e[r=1,family=bed,c=1] team run titleraw @s times 10 60 60
execute as @a if score @s team = @e[r=1,family=bed,c=1] team run titleraw @s title {"rawtext":[{"translate":"style.hypixel.trap_triggered.alarm_trap.title"}]}
execute as @a if score @s team = @e[r=1,family=bed,c=1] team run titleraw @s subtitle {"rawtext":[{"translate":"style.hypixel.trap_triggered.subtitle","with":{"rawtext":[{"translate":"team_upgrade.alarm_trap"}]}}]}
